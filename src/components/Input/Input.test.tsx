...
describe("Testing Input", () => {
  it("Match Snapshot", () => {
    const { asFragment } = render(<Input aria-label="input" />);
    expect(asFragment()).toMatchSnapshot();
  });

  it("Fire change event", () => {
    const { getByLabelText } = render(<Input aria-label="input" />);
    const element = getByLabelText("input") as HTMLInputElement;
  ...
  it("Fire change event for type number", () => {
    const { getByLabelText } = render(
      <Input aria-label="input-number" type="number" />
    );
    const element = getByLabelText("input-number") as HTMLInputElement;

    fireEvent.change(element, { target: { value: "Hello World!" } });
    expect(element.value).toBe("");

    fireEvent.change(element, { target: { value: "123435634.657658976" } });
    expect(element.value).toBe("123435634.657658976");

    fireEvent.change(element, { target: { value: "2348723hj2323" } });
    expect(element.value).toBe("");
  ...
  it("Check defalut value", () => {
    const { getByLabelText } = render(
      <Input aria-label="input" defaultValue="defalut value" />
    );
    const element = getByLabelText("input") as HTMLInputElement;
  ...
  it("Check fixed value", () => {
    const { getByLabelText } = render(
      <Input aria-label="input" value="lorem" />
    );
    const element = getByLabelText("input") as HTMLInputElement;
  ...
  it("Check onChange event", () => {
    ...
    const { getByLabelText } = render(
      <Input aria-label="input" onChange={onInputChange} />
    );
    const element = getByLabelText("input") as HTMLInputElement;
